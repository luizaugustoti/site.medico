...
// --------------------------------------------------------------------------
// TEMA — claro / escuro (o estado inicial já é definido inline no <head>
// do index.html para evitar flash de tema errado; aqui só tratamos o clique)
// --------------------------------------------------------------------------
const THEME_KEY = "frota-theme";

function aplicarTema(tema) {
  document.documentElement.setAttribute("data-theme", tema);
  try { localStorage.setItem(THEME_KEY, tema); } catch (e) { /* modo privado/sandbox: ignora */ }
}
...
function renderVeiculos() {
  ...
  if (!state.veiculos.length) {
    tbody.innerHTML = `<tr class="empty-row"><td colspan="5">Nenhum veículo cadastrado ainda. Clique em "Adicionar veículo".</td></tr>`;
    return;
  }
  // Nota: a tabela mostra apenas Equipe / Tipo / Placa / Motorista para leitura
  // rápida. Os demais campos (CPF, tela, CVV, implemento, rastreador,
  // redundância, diária) continuam sendo cadastrados e usados no resumo —
  // basta abrir "Editar" (✎) para ver e alterar o cadastro completo.
  tbody.innerHTML = state.veiculos.map(v => `
    <tr data-id="${v.id}">
      <td class="mono">${v.equipamento}</td>
      <td>${v.tipo}</td>
      <td class="mono">${v.placa}</td>
      <td class="driver-cell">${v.motorista || '<span class="empty">—</span>'}</td>
      <td class="col-actions">
        <div class="row-actions">
          <button class="btn-icon edit" title="Editar cadastro completo" onclick="abrirModalVeiculo('${v.id}')">✎</button>
