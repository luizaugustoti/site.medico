...
## Tema claro / escuro
Botão no topo (ícone de sol/lua) alterna entre os dois temas. A preferência
é salva no navegador (`localStorage`, chave `frota-theme`) e reaplicada nas
próximas visitas; sem preferência salva, o sistema segue o tema do
sistema operacional do usuário.

## Arquivos
- `index.html` — estrutura (tabela de veículos, tabela de operações com abas 1ª/2ª/3ª, modais de cadastro, alternador de tema)
- `style.css` — identidade visual da empresa (preto / verde / branco), com variáveis de tema claro e escuro
- `app.js` — estado, renderização, alternador de tema e a camada de API

## Tabela de veículos
A tabela principal mostra apenas **Equipe / Tipo / Placa / Motorista** para
leitura rápida. Os demais campos do cadastro (CPF, tela, CVV, implemento,
rastreador, redundância, diária) continuam existindo e alimentando o
resumo do topo — para vê-los ou editá-los, use o botão ✎ (editar) na linha
do veículo.
